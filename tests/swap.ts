...
describe("swap", async () => {
  ...
  it("Add liquid insufficient funds", async () => {
    // add liquid amount
    const rawAmount =
  ...
  it("Swap Token insufficient funds", async () => {
    const userBalance = await connection.getBalance(user.publicKey);
    let sig: string | null;
    try {
      sig = await program.methods
  ...
  it("Withdraw token insufficient funds", async () => {
    // add liquid amount
    let sig: string | null;
    try {
      const rawAmount = parseUnits(
  ...
  //////////// WITHDRAW NATIVE
  it("Withdraw native insufficient funds ", async () => {
    let sig: string | null;
    try {
      sig = await program.methods
